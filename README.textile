...
The sample app references the "Ariane":https://github.com/simonc/ariane lib directory via an initializer. The ariane repository must be located at the same directory level as the sample e.g. @~/Code/ariane@ and @~/Code/ariane-sample@.
...
TODO: Create an @ariane.return_to@ render method to generate a return_to html link that that can be used in addition to the break crumb trail to return to the previous task.
